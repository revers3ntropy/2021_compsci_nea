...
# ------------------------------------------------------------------------------------------------
#
#                  Diplsays the info for the menu option 'calculate profit'.
#
# ------------------------------------------------------------------------------------------------
#
# Imports:
...
def run():
    ...
    standard_seats = int(global_data.airplane_types_data[global_data.airplane_type][global_data.max_capasity]) - int(global_data.first_seats) * 2
    ...
    income = float(global_data.first_seats) * float(global_data.first_price) + standard_seats * float(global_data.standard_price)
    profit = income - cost

    typing.write(typing.retro_8x10, 'The flight costs ' + str(cost_per_seat) + ' per seat.', (renderer.mid_x, renderer.mid_y - 100))
    typing.write(typing.retro_8x10, 'The flight costs ' + str(cost) + ' in total.', (renderer.mid_x, renderer.mid_y - 50))
    typing.write(typing.retro_8x10, 'The flight has a total income of ' + str(income) + '.', (renderer.mid_x, renderer.mid_y))
    typing.write(typing.retro_8x10, "The flight's profit is " + str(profit) + '.', (renderer.mid_x, renderer.mid_y + 50))
